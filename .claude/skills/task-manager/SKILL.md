...
- **会議・レビュー・面談は独立タスクにしない**（親タスクの milestones / related_meetings で扱う。詳細は `.claude/rules/data-handling.md`）
- **粒度は粗めに**（学習済み / ins-20260529-002）: 同一担当・同一期限・同じ親タスクに収まる修正作業は1つの todo にまとめる。細かく分割しすぎない
